...
let
  ...
  buildNugetConfig = nugetSources:
    pkgs.stdenv.mkDerivation {
      ...
      installPhase =
        let
	  toCommand = name: params: ''${pkgs.dotnet-sdk}/bin/dotnet nuget add source "${params.url}" --name "${name}" --protocol-version "${builtins.toString(params.protocolVersion)}" --username "${params.username}" --password "${params.password}" --store-password-in-clear-text'';
